...
export async function GET() {
  ...
  if (process.env.E2E_TEST_MODE === "1" && userId === "e2e-user") {
    return NextResponse.json({
      ...
      habits: [{
        ...
        override_count: null, override_period: null, track_during_vacations: 0,
        project_name: "Training", color: "#ff6b57",
      }],
      completions: [],
      vacations: [],
    });
  ...
  const vacations = db.prepare("SELECT id,title,start_date,end_date FROM vacations WHERE user_id=? ORDER BY start_date").all(userId);
