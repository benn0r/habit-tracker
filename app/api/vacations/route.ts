...
export async function GET() {
  ...
  const db = getDb();
  const vacations = db
    .prepare("SELECT id,title,start_date,end_date FROM vacations WHERE user_id=? ORDER BY start_date DESC")
    .all(userId);
  const habits = db
    .prepare(
      "SELECT task_id,content,label_override,todoist_recurrence,override_type,override_count,track_during_vacations FROM habits WHERE user_id=? AND active=1 ORDER BY content",
    )
    .all(userId);
...
export async function POST(request: Request) {
  ...
  let vacation;
  try {
    vacation = parseVacationInput(await request.json());
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Invalid vacation" }, { status: 400 });
  }
  const result = getDb()
    .prepare("INSERT INTO vacations(user_id,title,start_date,end_date) VALUES(?,?,?,?)")
