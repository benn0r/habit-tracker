...
import { buildPeriods, rhythmFor, startOfDay, type Completion, type Period } from "@/lib/habit-periods";
...
const habitContextLabel = (habit: Habit) =>
  habit.project_name.trim().toLowerCase() === "inbox"
    ? scheduleLabel(habit)
    : `${habit.project_name} · ${scheduleLabel(habit)}`;

const displayLabel = (habit: Habit) => habit.label_override || habit.content;

const periodClass = (period: Period, tone: string) =>
  period.state === "vacation" || period.state === "before_start" ? "untracked" : tone;
...
export default function Dashboard() {
    ...
    fetch("/api/dashboard")
      ...
      .then((d) => {
        ...
        setSelected((current) => {
          ...
          return (
            current ||
            (requested && (requested === ALL_HABITS || d.habits?.some((h: Habit) => h.task_id === requested))
              ? requested
              : ALL_HABITS)
          );
        });
  ...
  useEffect(() => {
    ...
    // The URL is external state and must be applied once after client hydration.
  ...
  useEffect(() => {
    ...
    const restoreSelection = () => {
      const requested = new URLSearchParams(window.location.search).get("habit");
      setSelected(
        requested && (requested === ALL_HABITS || data.habits.some((h) => h.task_id === requested))
          ? requested
          : ALL_HABITS,
      );
      setMenuOpen(false);
  ...
  const detailAnalytics = useMemo(() => {
    ...
    const previous = all.filter(
      (period) =>
        period.date >= previousCutoff && period.date < cutoff && (period.state === "done" || period.state === "miss"),
    );
    ...
    const trend = Array.from({ length: bucketCount }, (_, index) => {
      ...
      return bucket.length
        ? Math.round((bucket.filter((period) => period.state === "done").length / bucket.length) * 100)
        : 0;
    });
  ...
  const monthly = useMemo(() => {
    ...
    for (const period of elapsed) {
      ...
      if (!month) {
        month = {
          key,
          label: period.date.toLocaleDateString(undefined, { month: "short" }),
          hit: 0,
          total: 0,
          score: 0,
        };
    ...
    () =>
      data?.habits.map((item) => {
        ...
        const previousElapsed = allPeriods.filter(
          (period) =>
            period.date >= previousCutoff &&
            period.date < dashboardCutoff &&
            (period.state === "done" || period.state === "miss"),
        );
        ...
        const previousBeforeStart =
          previousScore === null && trackingStart !== null && trackingStart >= dashboardCutoff;
        ...
        return {
          habit: item,
          periods: itemPeriods,
          tones: periodTones(itemPeriods.map((period) => period.state)),
          hits,
          total: itemElapsed.length,
          streak: itemStreak,
          score: itemScore,
          previousScore,
          previousBeforeStart,
          comparison: previousScore === null ? null : itemScore - previousScore,
          unit:
            rhythmFor(item).type === "weekly"
              ? "weeks"
              : rhythmFor(item).type === "interval"
                ? `${rhythmFor(item).count}-day periods`
                : "days",
          trend,
          stability,
        };
  ...
  if (!data)
    return (
      <div className="loading">
        <img className="brand-logo" src="/icons/favicon-rounded-192.png" alt="" width="34" height="34" />
        <p>Reading your habits…</p>
      </div>
    );
    ...
    <main className="shell">
      <aside className={menuOpen ? "menu-open" : ""}>
        ...
        <div className="side-content">
          ...
          <div className="habit-nav">
            <button className={`overview-link ${isOverview ? "active" : ""}`} onClick={() => selectHabit(ALL_HABITS)}>
            ...
            {data.habits.map((h) => (
              <button
                key={h.task_id}
                className={selected === h.task_id ? "active" : ""}
                onClick={() => selectHabit(h.task_id)}
              >
                <span className="habit-icon habit-dot" aria-hidden="true" />
                <span>
                  <strong>{displayLabel(h)}</strong>
                  <small>{scheduleLabel(h)}</small>
                </span>
              </button>
            ))}
          </div>
          <a className="vacations-link" href="/app/vacations">
            <span>☀</span>
            <span>
      ...
      <section className="dashboard">
        {isOverview ? (
          <>
            <header>
              <div>
                <div className="eyebrow">
                  <span /> {rangeLabel.toUpperCase()}
                </div>
                <h1>Your dashboard</h1>
                <p>A recent view of every rhythm you’re tracking.</p>
              </div>
              <button className="button ghost compact adjust-rhythm" onClick={sync} disabled={syncing}>
                <span className={syncing ? "spin" : ""}>↻</span>
                {syncing ? "Syncing…" : "Sync Todoist"}
              </button>
            </header>
            <div className="range-picker" role="group" aria-label="Dashboard time range">
            ...
            <div className="stats overview-stats">
              <article>
                ...
                <small>across all completed periods</small>
              </article>
              <article>
                <span>ACTIVE HABITS</span>
                <strong>{summaries.length}</strong>
                <small>currently tagged in Todoist</small>
              </article>
              <article>
                <span>TARGETS MET</span>
                <strong>{overviewHits}</strong>
                <small>in the last {rangeShort}</small>
              </article>
              <article>
                <span>NEEDS ATTENTION</span>
                <strong className={needsAttention ? "red" : ""}>{needsAttention}</strong>
                <small>habits below 60%</small>
              </article>
            </div>
            <article className={`portfolio-card ${portfolioDirection}`}>
              <div>
                <span>PORTFOLIO TREND</span>
                <h2>All habits, equally weighted</h2>
                <p>Each habit contributes the same weight, regardless of how often it is scheduled.</p>
              </div>
              <div className="portfolio-status">
                <strong>
                  {portfolioDirection === "improving"
                    ? "↗ Improving"
                    : portfolioDirection === "declining"
                      ? "↘ Declining"
                      : "→ Steady"}
                </strong>
                <small>
                  {portfolioDelta > 0 ? "+" : ""}
                  {portfolioDelta} points
                </small>
              </div>
              <svg
                viewBox="0 0 100 38"
                ...
                role="img"
                aria-label={`Portfolio trend ${portfolioDirection}`}
              >
                <path d="M0 19H100" />
                <polyline
                  points={portfolioTrend.map((value, index) => `${(index * 100) / 7},${35 - value * 0.32}`).join(" ")}
                />
              </svg>
            </article>
            <div className="habit-summary-grid">
              {summaries.map((summary) => (
                <button key={summary.habit.task_id} onClick={() => selectHabit(summary.habit.task_id)}>
                  <div className="summary-head">
                    <span className="habit-icon habit-dot" aria-hidden="true" />
                    <span>
                      <strong>{displayLabel(summary.habit)}</strong>
                      <small>{habitContextLabel(summary.habit)}</small>
                    </span>
                    <b>{summary.score}%</b>
                  </div>
                  ...
                  <div className="summary-metrics">
                    <div>
                      ...
                      >
                        {summary.previousBeforeStart
                          ? "Not tracked"
                          : summary.comparison === null
                            ? "No data"
                            : `${summary.comparison > 0 ? "+" : ""}${summary.comparison} pts`}
                      </strong>
                      <small>
                        {summary.previousBeforeStart
                          ...
                          : summary.previousScore === null
                            ...
                            : `${summary.previousScore}% previously`}
                      </small>
                    </div>
                    <div>
                      <span>Stability</span>
                      <strong>{summary.stability}/100</strong>
                      <small>
                        {summary.stability >= 80 ? "Consistent" : summary.stability >= 60 ? "Mixed" : "Volatile"}
                      </small>
                    </div>
                  </div>
                  <div className="summary-recent">
                    {summary.periods.map((period, index) => (
                      <i
                        key={period.key}
                        className={periodClass(period, summary.tones[index])}
                        data-period-state={period.state}
                        data-period-tone={summary.tones[index]}
                        aria-label={period.label}
                        onMouseEnter={(event) => showPeriodTooltip(period.label, event.currentTarget)}
                        onMouseLeave={() => setPeriodTooltip(null)}
                      />
                    ))}
                  </div>
                  <div className="summary-foot">
                    <span>{summary.hits} targets met</span>
                    <span>
                      {summary.streak} {summary.unit} streak
                    </span>
                    <strong>View habit →</strong>
                  </div>
                </button>
              ))}
            </div>
            <p className="last-sync">
              Last synced {data.user.last_sync ? new Date(data.user.last_sync).toLocaleString() : "never"}
            </p>
          </>
        ) : habit ? (
          <>
            <header>
              <div>
                ...
                <h1>{displayLabel(habit)}</h1>
                <p>{habitContextLabel(habit)}</p>
              </div>
              <button className="button ghost compact adjust-rhythm" onClick={openSettings}>
            ...
            <div className="stats">
              <article>
                ...
                <small>across completed periods</small>
              </article>
              <article>
                <span>SUCCESSFUL PERIODS</span>
              ...
              <article>
                ...
                <small>{streak ? "keep the rhythm going" : "this period is a fresh start"}</small>
              </article>
              <article>
                <span>MISSED</span>
                <strong className="red">{elapsed.length - completed}</strong>
                <small>{unit} below target</small>
              </article>
            </div>
            <article className={`detail-analytics ${detailAnalytics.direction}`}>
              <div className="detail-analytic-copy">
                <span>12-MONTH TREND</span>
                <strong>
                  {detailAnalytics.direction === "improving"
                    ? "↗ Improving"
                    : detailAnalytics.direction === "declining"
                      ? "↘ Declining"
                      : "→ Steady"}
                </strong>
                <small>
                  {detailAnalytics.delta > 0 ? "+" : ""}
                  {detailAnalytics.delta} points from early to recent periods
                </small>
              </div>
              <svg
                viewBox="0 0 100 34"
                preserveAspectRatio="none"
                role="img"
                aria-label={`${detailAnalytics.direction} yearly trend`}
              >
                <path d="M0 17H100" />
                <polyline
                  points={detailAnalytics.trend
                    .map(
                      (value, index) =>
                        `${detailAnalytics.trend.length === 1 ? 50 : (index * 100) / (detailAnalytics.trend.length - 1)},${31 - value * 0.28}`,
                    )
                    .join(" ")}
                />
              </svg>
              <div className="detail-analytic-stat">
                <span>STABILITY</span>
                <strong>
                  {detailAnalytics.stability}
                  <em>/100</em>
                </strong>
                <small>
                  {detailAnalytics.stability >= 80
                    ? "Consistent"
                    : detailAnalytics.stability >= 60
                      ? "Mixed"
                      : "Volatile"}
                </small>
              </div>
              <div className="detail-analytic-stat">
                ...
                <strong
                  className={
                    detailAnalytics.comparison === null
                      ? ""
                      : detailAnalytics.comparison > 0
                        ? "positive"
                        : detailAnalytics.comparison < 0
                          ? "negative"
                          : ""
                ...
                >
                  {detailAnalytics.comparison === null
                    ? "—"
                    : `${detailAnalytics.comparison > 0 ? "+" : ""}${detailAnalytics.comparison}`}
                  <em>{detailAnalytics.comparison === null ? "" : " pts"}</em>
                </strong>
                <small>
                  {detailAnalytics.previousScore === null
                    ? "No previous data yet"
                    : `${detailAnalytics.previousScore}% in the prior 12 months`}
                </small>
              </div>
            </article>
            ...
            <article className="chart-card">
              <div className="chart-title">
                ...
                {view === "heatmap" && (
                  <div className="chart-legend">
                    <i className="done" /> Target met <i className="warning" /> First miss <i className="miss" />{" "}
                    Repeated miss <i className="untracked" /> Untracked <i className="none" /> Current period
                  </div>
                )}
              </div>
              ...
              {view === "heatmap" && (
                <div className="heatmap-layout">
                  ...
                  <div className={`heatmap-scroll ${rhythm.type}`}>
                    <div className={`year-grid ${rhythm.type}`}>
                      {periods.map((period, index) => (
                        <i
                          key={period.key}
                          className={periodClass(period, tones[index])}
                          data-period-state={period.state}
                          data-period-tone={tones[index]}
                          aria-label={period.label}
                          onMouseEnter={(event) => showPeriodTooltip(period.label, event.currentTarget)}
                          onMouseLeave={() => setPeriodTooltip(null)}
                        >
                          {rhythm.type === "weekly" ? period.completed : null}
                        </i>
                      ))}
                    </div>
                  </div>
              ...
              {view === "trend" && (
                <div className="trend-chart">
                  {monthly.map((month) => (
                    <div className="trend-month" key={month.key} title={`${month.hit}/${month.total} targets met`}>
                      <strong>{month.score}%</strong>
                      <div>
                        <i style={{ height: `${Math.max(month.score, 3)}%` }} />
                      </div>
              ...
              {view === "history" && (
                <div className="period-history">
                  {periods
                    ...
                    .map((period, index) => ({
                      period,
                      tone: tones[periods.length - Math.min(18, periods.length) + index],
                    }))
                    .reverse()
                    .map(({ period, tone }) => (
                      <div key={period.key}>
                        <i className={periodClass(period, tone)} />
                        <span>
                          <strong>{period.label.split(":")[0]}</strong>
                          <small>
                            {period.state === "before_start"
                              ? "Before tracking started"
                              : period.state === "vacation"
                                ? "Not tracked during vacation"
                                : period.state === "future"
                                  ? "Still in progress"
                                  : `${period.completed} of ${period.target} completed`}
                          </small>
                        </span>
                        <b>
                          {period.state === "done"
              ...
              <div className="insight">
                ...
                <p>
                  <strong>
                    {score >= 80 ? "Strong rhythm." : score >= 55 ? "A rhythm is forming." : "Room to reset."}
                  </strong>{" "}
                  You hit your target in {completed} {unit} this year. Misses are information, not failure.
                </p>
              </div>
            </article>
            <p className="last-sync">
          ...
          <div className="empty">
            <span>✓</span>
            <h1>No habits found yet</h1>
            <p>
              Add the <code>@habit</code> label to a recurring Todoist task, then sync.
            </p>
      ...
      {periodTooltip && (
        <div className="period-tooltip" role="tooltip" style={{ left: periodTooltip.x, top: periodTooltip.y }}>
          <strong>{periodTooltip.title}</strong>
          <small>{periodTooltip.detail}</small>
        </div>
      )}
      {toast && (
      ...
      {settings && habit && (
        <div className="modal-bg" onMouseDown={() => setSettings(false)}>
          <div className="modal settings-modal" onMouseDown={(e) => e.stopPropagation()}>
            <button className="close" aria-label="Close settings" onClick={() => setSettings(false)}>
              ×
            </button>
            <div className="eyebrow">
              <span /> HABIT SETTINGS
            ...
            <section className="settings-section label-setting">
              <div className="settings-heading">
                <strong>Label</strong>
                <small>Todoist name: {habit.content}</small>
              </div>
              <div className="label-control">
                <input
                  ...
                  maxLength={80}
                  value={labelOverride}
                  placeholder={habit.content}
                  onChange={(event) => setLabelOverride(event.target.value)}
                />
                <button className="button primary compact" onClick={saveLabel}>
                  Save label
                </button>
              </div>
            ...
            <section className="settings-section vacation-setting">
              <div>
                <strong>Track during vacations</strong>
                <small>Off by default. Vacation periods are grey and excluded from analytics.</small>
              </div>
              <label className="toggle">
                <input
                  type="checkbox"
                  aria-label="Track during vacations"
                  checked={Boolean(habit.track_during_vacations)}
                  onChange={(event) => saveVacationTracking(event.target.checked)}
                />
                <span />
              </label>
            </section>
            <section className="settings-section start-date-setting">
              <div className="settings-heading">
                ...
                <small>Earlier periods are grey and excluded</small>
              </div>
              <div className="label-control">
                <input
                ...
                />
                <button className="button primary compact" onClick={saveTrackingStartDate}>
                  Save date
                </button>
              </div>
              <small>
                {trackingStartDate ? "Only periods from this date count" : "No start date — all history counts"}
              </small>
            </section>
            <section className="settings-section rhythm-setting">
              <div className="settings-heading">
                <strong>Rhythm</strong>
                <small>How often this habit should count</small>
              </div>
              <div className="choices primary-choices">
                <button className={scheduleIs("todoist") ? "selected" : ""} onClick={() => saveSchedule("todoist")}>
                  <i>↻</i>
                  <span>
                    <strong>Use Todoist schedule</strong>
                    <small>{habit.todoist_recurrence || "No recurring due date"}</small>
                  </span>
                  <b>✓</b>
                </button>
                <button className={scheduleIs("daily") ? "selected" : ""} onClick={() => saveSchedule("daily")}>
                  <i>1d</i>
                  <span>
                    <strong>Every day</strong>
                    <small>One completion each day</small>
                  </span>
                  <b>✓</b>
                </button>
                <button
                  className={scheduleIs("interval", 2) ? "selected" : ""}
                  onClick={() => saveSchedule("interval", 2)}
                >
                  <i>2d</i>
                  <span>
                    <strong>Every two days</strong>
                    <small>One completion per two-day period</small>
                  </span>
                  <b>✓</b>
                </button>
                {WEEKLY_OPTIONS.map((label, index) => {
                  const count = index + 1;
                  return (
                    <button
                      className={scheduleIs("weekly", count) ? "selected" : ""}
                      key={count}
                      onClick={() => saveSchedule("weekly", count)}
                    >
                      <i>{count}×</i>
                      <span>
                        <strong>{label}</strong>
                        <small>Any days, Monday–Sunday</small>
                      </span>
                      <b>✓</b>
                    </button>
                  );
