...
const habits = [
  ...
  task_id, content, todoist_recurrence, project_name,
  label_override: null, override_type: null, override_count: null,
  override_period: null, color: ["#4f8ac9", "#8d6cc4", "#e05c45", "#d39c3f", "#4e8b67", "#566b9f"][index],
}));
...
function fantasyCompletions() {
  ...
  for (let daysAgo = 1; daysAgo <= 400; daysAgo++) {
    ...
    if ([1, 2, 4, 6].includes(date.getDay()) && daysAgo % 19 !== 0) completions.push({ task_id: "dragon", completed_at });
...
test("captures the fantasy-data app dashboard for the README", async ({ context, page }, testInfo) => {
  ...
  await page.route("**/api/dashboard", (route) => route.fulfill({
    ...
    body: JSON.stringify({
      user: { name: "Lyra Moonwhisper", email: "lyra@moonkeep.example", last_sync: new Date().toISOString() },
      habits,
