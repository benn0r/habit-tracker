...
export function parseVacationInput(value: unknown) {
  ...
  if (title.length > MAX_VACATION_TITLE_LENGTH) throw new Error(`Title must be ${MAX_VACATION_TITLE_LENGTH} characters or fewer`);
  if (typeof body.startDate !== "string" || !validDateString(body.startDate)) throw new Error("Valid start date is required");
...
export function periodOverlapsVacation(periodStart: Date, periodEnd: Date, vacations: Pick<Vacation, "start_date" | "end_date">[]) {
